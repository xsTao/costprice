...
import nc.bs.mapub.materialpricebase.rule.PriceSourceRule;
...
/**
 * @since v6.3
 * @version 2015年7月13日 上午11:00:40
 * @author zhangshyb
 */
@SuppressWarnings({
    "unchecked", "rawtypes"
})
public class MaterialPriceBaseImportInsertBP {

    public MaterialPriceBaseAggVO[] insert(MaterialPriceBaseAggVO[] bills) {
        InsertBPTemplate<MaterialPriceBaseAggVO> bp =
                new InsertBPTemplate<MaterialPriceBaseAggVO>(MaterialPriceBasePluginPoint.IMPORT_INSERT);
    ...
    private void addBeforeRule(AroundProcesser<MaterialPriceBaseAggVO> processer) {
        ...
        // 校验价格来源字段的合法性
        IRule<MaterialPriceBaseAggVO> priceSourceRule = new PriceSourceRule();
        processer.addBeforeRule(priceSourceRule);
