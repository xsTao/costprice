...
import nc.bs.mapub.materialpricebase.rule.CancleLockRule;
...
public class MaterialPriceBaseDeblockingBP {
    public MaterialPriceBaseAggVO[] deblocking(MaterialPriceBaseAggVO[] bills, MaterialPriceBaseAggVO[] originBills) {
        // 调用修改模板
        UpdateBPTemplate<MaterialPriceBaseAggVO> bp =
                new UpdateBPTemplate<MaterialPriceBaseAggVO>(MaterialPriceBasePluginPoint.DEBLOCKING);
        // 执行前规则
    ...
    private void addBeforeRule(CompareAroundProcesser<MaterialPriceBaseAggVO> processer) {
        ...
        // 取消锁定信息
        IRule<MaterialPriceBaseAggVO> cancleLockRule = new CancleLockRule();
        processer.addBeforeFinalRule(cancleLockRule);
