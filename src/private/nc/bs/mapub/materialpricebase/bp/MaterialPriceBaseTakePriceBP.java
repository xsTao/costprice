...
import nc.bs.mapub.materialpricebase.rule.TakePriceRule;
...
public class MaterialPriceBaseTakePriceBP {
    public MaterialPriceBaseAggVO[] takePrice(MaterialPriceBaseAggVO[] bills, MaterialPriceBaseAggVO[] originBills) {
        // 调用修改模板
        UpdateBPTemplate<MaterialPriceBaseAggVO> bp =
                new UpdateBPTemplate<MaterialPriceBaseAggVO>(MaterialPriceBasePluginPoint.TAKEPRICE);
        // 执行前规则
    ...
    private void addBeforeRule(CompareAroundProcesser<MaterialPriceBaseAggVO> processer) {
        ...
        // 添加取价rule
        IRule<MaterialPriceBaseAggVO> takePriceRule = new TakePriceRule();
        processer.addBeforeRule(takePriceRule);
